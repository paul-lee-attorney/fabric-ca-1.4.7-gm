...
func (tm *Mgr) GetBatch(req *GetTCertBatchRequest, ecert *x509.Certificate) (*api.GetTCertBatchResponse, error) {
	...
	raw, _ := gmx509.MarshalPKIXSM2PublicKey(pub)
	mac.Write(raw)
	kdfKey := mac.Sum(nil)
	...
	for i := 0; i < numTCertsInBatch; i++ {
		...
		raw, err := gmx509.CreateCertificate(rand.Reader, template, tm.CACert, &txPub, tm.CAKey)
		if err != nil {
			return nil, fmt.Errorf("Failed in TCert gmx509.CreateCertificate: %s", err)
		}
