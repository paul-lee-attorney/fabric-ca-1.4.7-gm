...
func genRoot(name, dir string) ([]byte, *rsa.PrivateKey, error) {
	...
	var parent = template
	cert, err := gmx509.CreateCertificate(rand.Reader, template, parent, publickey, privatekey)
	if err != nil {
		return nil, nil, err
	}
...
func genCert(name, dir string, cert []byte, privKey *rsa.PrivateKey) error {
	...
	parentCert, err := gmx509.ParseCertificate(cert)

	// create a self-signed certificate. template = parent
	cert, err = gmx509.CreateCertificate(rand.Reader, template, parentCert, publickey, privKey)
	if err != nil {
		return err
	}
