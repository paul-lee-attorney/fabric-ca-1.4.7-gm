...
func EncodeKeys(privateKey *sm2.PrivateKey, publicKey *sm2.PublicKey) ([]byte, []byte, error) {
	...
	encodedPK, err := gmx509.MarshalSM2PrivateKey(privateKey)
	if err != nil {
		return nil, nil, errors.Wrap(err, "Failed to encode ECDSA private key")
	}
	pemEncodedPK := pem.EncodeToMemory(&pem.Block{Type: "SM2 PRIVATE KEY", Bytes: encodedPK})

	encodedPubKey, err := gmx509.MarshalPKIXSM2PublicKey(publicKey)
	if err != nil {
		return nil, nil, errors.Wrap(err, "Failed to encode ECDSA public key")
	}
	pemEncodedPubKey := pem.EncodeToMemory(&pem.Block{Type: "SM2 PUBLIC KEY", Bytes: encodedPubKey})
...
func DecodeKeys(pemEncodedPK, pemEncodedPubKey []byte) (*ecdsa.PrivateKey, *ecdsa.PublicKey, error) {
	...
	pk, err := gmx509.ParseSM2PrivateKey(block.Bytes)
	if err != nil {
	...
	key, err := gmx509.ParsePKIXSM2PublicKey(blockPub.Bytes)
	if err != nil {
