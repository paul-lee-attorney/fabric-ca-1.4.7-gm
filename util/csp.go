...
import (
	...
	cspsigner "github.com/paul-lee-attorney/fabric-2.1-gm/bccsp/signer"
...
func GetSignerFromCertFile(certFile string, csp bccsp.BCCSP) (bccsp.Key, crypto.Signer, *x509.Certificate, error) {
	...
	if block == nil {
		return nil, nil, nil, errors.New("certificate does not in form of PEM")
	}

	if block.Type != "SM2 CERTIFICATE" {
		return nil, nil, nil, errors.New("certificate is not a SM2 one")
	}

	parsedCa, err := sm2cert.ParseCertificate(block.Bytes)
	if err != nil {
		return nil, nil, nil, err
	}
...
func ImportBCCSPKeyFromPEM(keyFile string, myCSP bccsp.BCCSP, temporary bool) (bccsp.Key, error) {
	...
	//加入国密SM2私钥导入
	if key, err := gm.PEMtoSM2PrivateKey(keyBuff, nil); err == nil {
		switch key.(type) {
		case *sm2.PrivateKey:
			priv, err := gm.MarshalPKCS8SM2PrivateKey(key.(*sm2.PrivateKey))
			if err != nil {
...
func LoadX509KeyPair(certFile, keyFile string, csp bccsp.BCCSP) (*tls.Certificate, error) {
	...
	for {
		...
		if certDERBlock.Type == "SM2 CERTIFICATE" {
	...
	if len(cert.Certificate) == 0 {
		...
		if len(skippedBlockTypes) == 1 && strings.HasSuffix(skippedBlockTypes[0], "SM2 PRIVATE KEY") {
		...
		return nil, errors.Errorf("Failed to find \"SM2 CERTIFICATE\" PEM block in file %s after skipping PEM blocks of the following types: %v", certFile, skippedBlockTypes)
	}
